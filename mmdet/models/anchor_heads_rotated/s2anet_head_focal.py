...
class WeightedFocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets, weight, avg_factor):
        #print(f'targets.shape: {targets.shape}')
        #print(f'inputs.shape: {inputs.shape}')
        p_t = torch.sigmoid(inputs)
        p_t = (targets * p_t) + ((1-targets) * (1 - p_t))
        F_loss = - self.alpha * ((1-p_t)**self.gamma) * torch.log(p_t) * weight
        return F_loss.sum() / avg_factor


@HEADS.register_module
class S2ANetHeadFocal(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_odm_xwh_bbox=dict(
                     type='SmoothL1Loss', beta=1.0 / 9.0, loss_weight=1.0)):
        super(S2ANetHeadFocal, self).__init__()
    ...
    def _loss_dcl_single(self, gt_theta, gt_h, gt_w, pred_theta, weight, avg_factor):
        ...
        # )
        BinaryFocalLoss = WeightedFocalLoss()
        f_loss = BinaryFocalLoss(pred_theta, gt_bin_theta, weight, avg_factor)
