...
class S2ANetHeadClsWeighted(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_odm_xwh_bbox=dict(
                     type='SmoothL1Loss', beta=1.0 / 9.0, loss_weight=1.0)):
        super(S2ANetHeadClsWeighted, self).__init__()
    ...
    def _loss_dcl_single(self, gt_theta, gt_h, gt_w, pred_theta, weight, avg_factor):
        ...
        cls_weights = torch.Tensor([128, 64, 32, 16, 8, 4, 2, 1]).to(pred_theta.device)
        cls_weights = torch.sqrt(cls_weights/128)
        f_loss = cls_weights * f_loss
        f_loss = f_loss.sum() / avg_factor
